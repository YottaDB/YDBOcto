...
+++++++++++++++++
  ...
  |                 |                    |               | PRIMARY KEY instead.                                                           |                              |                                                           |
  +-----------------+--------------------+---------------+--------------------------------------------------------------------------------+------------------------------+-----------------------------------------------------------+
  | MAYBE_CANONICAL | Not applicable     | Column        | Indicates that, if this column is declared to be a VARCHAR type, the table     | Not applicable               | Not applicable                                            |
  |                 |                    |               | could contain values for this column which are a mix of                        |                              |                                                           |
  |                 |                    |               | canonical and non-canonical numbers. The presence of this keyword disables     |                              |                                                           |
  |                 |                    |               | some optimizations as incorrect results would otherwise be returned (see       |                              |                                                           |
  |                 |                    |               | `here <https://gitlab.com/YottaDB/DB/YDB/-/issues/1031#note_1600678857>`_      |                              |                                                           |
  |                 |                    |               | for more details). An example column that would require this keyword is the    |                              |                                                           |
  |                 |                    |               | :code:`zipcode` column. The first 5 digits are numeric but in case the first   |                              |                                                           |
  |                 |                    |               | digit is not a :code:`0`, the value would be treated as numeric by Octo even   |                              |                                                           |
  |                 |                    |               | though the column was declared to be of type :code:`VARCHAR` in the CREATE     |                              |                                                           |
  |                 |                    |               | TABLE command. If the zipcode starts with a :code:`0`, the value would be      |                              |                                                           |
  |                 |                    |               | treated as a string by Octo. This difference in treatment would result in      |                              |                                                           |
