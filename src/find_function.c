...
SqlFunction *find_function(const char *function_name) {
	...
	status = ydb_get_s(&loaded_schemas, 2, &function_subs[0], &ret);
	switch (status) {
	case YDB_OK:
		...
		if (!drop_cache) {
			...
			status = drop_schema_from_local_cache(&function_subs[1], FunctionSchema);
			if (YDB_OK != status) {
	...
	if (YDB_ERR_GVUNDEF == status) {
		...
		ERROR(ERR_UNKNOWN_FUNCTION, function_name);
		return NULL;
	}
	YDB_ERROR_CHECK(status);
	if (YDB_OK != status) {
	...
	function_subs[3].buf_addr[function_subs[3].len_used] = '\0';
	length_long = strtol(function_subs[3].buf_addr, NULL, 10);
	...
	while (TRUE) {
		...
		if (YDB_OK != status)
			break;
		status = ydb_get_s(&octo_global, 4, &function_subs[0], &value_buffer);
		YDB_ERROR_CHECK(status);
		if (YDB_OK != status)
			break;
