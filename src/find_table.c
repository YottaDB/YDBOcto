...
SqlTable *find_table(const char *table_name) {
	...
	ydb_buffer_t	*save_buffers;
	ydb_buffer_t	*save_chunk;
	ydb_buffer_t	varname, subs_array[2], ret;
	...
	status = ydb_get_s(&varname, 1, subs_array, &ret);
	switch(status) {
	case YDB_OK:
		...
		/* subs_array[0] is already initialized */
		YDB_LITERAL_TO_BUFFER("pg_class", &subs_array[1]);
		ret.buf_addr = &oid_buff[0];
		ret.len_alloc = sizeof(oid_buff);
		status = ydb_get_s(&varname, 2, subs_array, &ret);
		switch(status) {
		case YDB_OK:
		...
		if (!drop_cache) {
			...
			status = drop_table_from_local_cache(&subs_array[0]);
			if (YDB_OK != status) {
	...
	/* Switch gvn 2nd subscfript from "l" back to "b" to get the binary table definition */
	...
	cur_buff = buff;
	while (TRUE) {
		/* See "src/run_query.c" under "case table_STATEMENT:" for how these global variable nodes are created */
	...
	save_buffers = make_buffers(config->global_names.loadedschemas, 1, table_name);
	save_chunk = make_buffers(config->global_names.loadedschemas, 2, table_name, "chunk");
