 ...
 * `set_oper_stmt` and its `col_type_list` to identify the type instead of deriving the
 * type from the `column` field. This is because `col_type_list` is set with type information
...
SqlValueType get_set_operation_column_alias_type(SqlStatement *ca_stmt) {
	...
	start_cla = cur_cla = set_oper->col_type_list;
