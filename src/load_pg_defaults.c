...
#define LOAD_PG_VARIABLE(NAME, NAME_UPPER, ROW_DEFAULT)                                                              \
	{                                                                                                            \
		int	     status;                                                                                 \
		ydb_buffer_t pg_buffers[5];                                                                          \
		ydb_buffer_t value_buffer;                                                                           \
                                                                                                                     \
		/* Map the canonical runtime parameter name, i.e. the case-sensitive name used by PostgreSQL, */     \
		/* to the lowercase form of that name for later case-insensitive lookup during SET/SHOW commands. */ \
		...
		YDB_STRING_TO_BUFFER(NAME_UPPER, &pg_buffers[3]);                                                    \
		YDB_STRING_TO_BUFFER(NAME, &value_buffer);                                                           \
                                                                                                                     \
...
#define LOAD_READ_ONLY_VARIABLE(NAME_UPPER, DEFAULT_VALUE)                                                             \
	{                                                                                                              \
		...
		/*	%ydboctoocto(OCTOLIT_SETTINGS,OCTOLIT_READ_ONLY,NAME_UPPER)=DEFAULT_VALUE */                        \
		YDB_STRING_TO_BUFFER(config->global_names.raw_octo, &pg_buffers[0]);                                   \
		YDB_STRING_TO_BUFFER(OCTOLIT_SETTINGS, &pg_buffers[1]);                                                \
