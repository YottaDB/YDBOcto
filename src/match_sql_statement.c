...
boolean_t match_sql_statement(SqlStatement *stmt, SqlStatement *match_stmt) {
	...
	case table_alias_STATEMENT:
		...
		ret = match_sql_statement(table_alias->table, match_table_alias->table); if (!ret) break;
		/* Consider two queries with the same table but different aliases as identical hence "table_alias->alias"
		 * is not checked here.
		 */
	...
	case select_STATEMENT:
		...
		ret = match_sql_statement(select->select_list, match_select->select_list); if (!ret) break;
		ret = match_sql_statement(select->table_list, match_select->table_list); if (!ret) break;
		ret = match_sql_statement(select->where_expression, match_select->where_expression); if (!ret) break;
		ret = match_sql_statement(select->group_by_expression, match_select->group_by_expression); if (!ret) break;
		ret = match_sql_statement(select->having_expression, match_select->having_expression); if (!ret) break;
		ret = match_sql_statement(select->optional_words, match_select->optional_words); if (!ret) break;
	...
	case value_STATEMENT:
		...
		ret = (value->type == match_value->type); if (!ret) break;
		if (COERCE_TYPE == value->type) {
			ret = (value->pre_coerced_type == match_value->pre_coerced_type); if (!ret) break;
			ret = (value->coerced_type == match_value->coerced_type); if (!ret) break;
		}
		...
		if (CALCULATED_VALUE == value->type) {
			ret = match_sql_statement(value->v.calculated, match_value->v.calculated); if (!ret) break;
		} else if (NUL_VALUE == value->type) {
			// No need to match any more for NULL value
		} else {
			ret = !strcmp(value->v.reference, match_value->v.reference); if (!ret) break;
		}
		break;
	case binary_STATEMENT:
		UNPACK_SQL_STATEMENT(binary, stmt, binary);
		UNPACK_SQL_STATEMENT(match_binary, match_stmt, binary);
		ret = match_sql_statement(binary->operands[0], match_binary->operands[0]); if (!ret) break;
		ret = match_sql_statement(binary->operands[1], match_binary->operands[1]); if (!ret) break;
		break;
	case unary_STATEMENT:
		UNPACK_SQL_STATEMENT(unary, stmt, unary);
		UNPACK_SQL_STATEMENT(match_unary, match_stmt, unary);
		ret = match_sql_statement(unary->operand, match_unary->operand); if (!ret) break;
		break;
	case column_list_STATEMENT:
		...
		ret = ((NULL == cur_column_list) == (NULL == match_cur_column_list)); if (!ret) break;
		ret = (NULL == cur_column_list); if (ret) break;	/* Note: "if (ret)" used here instead of
									 * the usual "if (!ret)".
									 */
		do {
			ret = match_sql_statement(cur_column_list->value, match_cur_column_list->value); if (!ret) break;
			cur_column_list = cur_column_list->next;
			match_cur_column_list = match_cur_column_list->next;
			ret = ((start_column_list == cur_column_list)
					== (match_start_column_list == match_cur_column_list)); if (!ret) break;
	...
	case column_list_alias_STATEMENT:
		...
		ret = ((NULL == cur_cl_alias) == (NULL == match_cur_cl_alias)); if (!ret) break;
		ret = (NULL == cur_cl_alias); if (ret) break;	/* Note: "if (ret)" used here instead of
								 * the usual "if (!ret)".
								 */
		do {
			ret = (cur_cl_alias->type == match_cur_cl_alias->type); if (!ret) break;
			ret = match_sql_statement(cur_cl_alias->column_list, match_cur_cl_alias->column_list); if (!ret) break;
			ret = match_sql_statement(cur_cl_alias->alias, match_cur_cl_alias->alias); if (!ret) break;
			ret = match_sql_statement(cur_cl_alias->keywords, match_cur_cl_alias->keywords); if (!ret) break;
			cur_cl_alias = cur_cl_alias->next;
			match_cur_cl_alias = match_cur_cl_alias->next;
			ret = ((start_cl_alias == cur_cl_alias) == (match_start_cl_alias == match_cur_cl_alias)); if (!ret) break;
	...
	case join_STATEMENT:
		...
		do {
			ret = (cur_join->type == match_cur_join->type); if (!ret) break;
			ret = match_sql_statement(cur_join->value, match_cur_join->value); if (!ret) break;
			ret = match_sql_statement(cur_join->condition, match_cur_join->condition); if (!ret) break;
			cur_join = cur_join->next;
			match_cur_join = match_cur_join->next;
			ret = ((start_join == cur_join) == (match_start_join == match_cur_join)); if (!ret) break;
	...
	case keyword_STATEMENT:
		...
		do {
			ret = (cur_keyword->keyword == match_cur_keyword->keyword); if (!ret) break;
			ret = match_sql_statement(cur_keyword->v, match_cur_keyword->v); if (!ret) break;
			cur_keyword = cur_keyword->next;
			match_cur_keyword = match_cur_keyword->next;
			ret = ((start_keyword == cur_keyword) == (match_start_keyword == match_cur_keyword)); if (!ret) break;
	...
	case cas_STATEMENT:
		...
		ret = match_sql_statement(cas->value, match_cas->value); if (!ret) break;
		ret = match_sql_statement(cas->branches, match_cas->branches); if (!ret) break;
		ret = match_sql_statement(cas->optional_else, match_cas->optional_else); if (!ret) break;
		break;
	case cas_branch_STATEMENT:
		...
		do {
			ret = match_sql_statement(cur_cas_branch->condition, match_cur_cas_branch->condition); if (!ret) break;
			ret = match_sql_statement(cur_cas_branch->value, match_cur_cas_branch->value); if (!ret) break;
			cur_cas_branch = cur_cas_branch->next;
			match_cur_cas_branch = match_cur_cas_branch->next;
			ret = ((start_cas_branch == cur_cas_branch)
					== (match_start_cas_branch == match_cur_cas_branch)); if (!ret) break;
	...
	case function_call_STATEMENT:
		...
		ret = match_sql_statement(function_call->function_name, match_function_call->function_name); if (!ret) break;
		ret = match_sql_statement(function_call->parameters, match_function_call->parameters); if (!ret) break;
		break;
	case aggregate_function_STATEMENT:
		...
		ret = (aggregate_function->type == match_aggregate_function->type); if (!ret) break;
		ret = match_sql_statement(aggregate_function->parameter, match_aggregate_function->parameter); if (!ret) break;
		break;
	default:
		assert(FALSE);
