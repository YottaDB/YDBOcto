...
/* Function invoked by src/qualify_query.
 *
 * Returns:
...
int copy_correlation_specification_aliases(SqlTableAlias *table_alias) {
	...
	assert(NULL != table_alias->correlation_specification);
	/* `correlation_specification` is a pointer to a table name alias and an optional list of column name aliases. */
	UNPACK_SQL_STATEMENT(table_name_alias, table_alias->correlation_specification, column_list);
