...
int emit_physical_or_xref_plan(char *plan_filename, SqlStatement *stmt, char *tableName, char *columnName,
	...
	for (i = 0;; i++) {
		...
		/* Store output key for the given plan. Note that for a LP_INSERT_INTO/LP_DELETE_FROM/LP_UPDATE plan, there is no
		...
		if (NULL != set_oper) {
			...
			/* The only cases we know of are an XREF/INSERT INTO/DELETE FROM/UPDATE plan. They are asserted below.
			...
			output_key_id = ZERO_OUTPUT_KEY_ID;
		} else {
