...
int handle_query_response(SqlStatement *stmt, ydb_long_t cursorId, void *_parms, char *plan_name, PSQL_MessageTypeT msg_type) {
	...
	if (NULL != stmt) {
		...
		if (select_STATEMENT == stmt->type) {
			...
			/* It is of type INSERT INTO, DELETE FROM, UPDATE etc. In that case, there are no rows to send back.
			 * Just update "parms->row_count" to reflect the number of rows inserted, deleted, updated etc..
			 */
