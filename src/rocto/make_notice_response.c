...
NoticeResponse *make_error_response(PSQL_ErrorSeverity severity, PSQL_SQLSTATECode code, const char *message, size_t num_args, ...) {
	...
	for(i = 0; i < num_args; i++) {
		arg = va_arg(args, NoticeResponseArg*);
		// type argument
		new_length += 1;
	...
	new_length += 1;
	ret = (NoticeResponse*)malloc(sizeof(NoticeResponse) + new_length);
	memset(ret, 0, sizeof(NoticeResponse) + new_length);

	// This is mostly for testing; eventually, we should
	//  check whether this is a debug build, and if so, not do this maloc
	//  or the sets
	ret->args = (NoticeResponseArg*)malloc(sizeof(NoticeResponseArg) * (num_args+3));
	cur_arg = 0;
