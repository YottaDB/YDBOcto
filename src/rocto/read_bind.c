...
Bind *read_bind(BaseMessage *message) {
	...
	if (ret->num_parm_format_codes < 0) {
		ERROR(ERR_INVALID_NUMBER, "Bind", "parameter format codes", ret->num_parm_format_codes, 0, INT16_MAX);
		free(ret);
		return NULL;
	}
	...
	if (ret->num_result_col_format_codes < 0) {
		ERROR(ERR_INVALID_NUMBER, "Bind", "result column format codes", ret->num_result_col_format_codes, 0, INT16_MAX);
		free(ret);
		return NULL;
	}
	cur_pointer += sizeof(int16_t);
