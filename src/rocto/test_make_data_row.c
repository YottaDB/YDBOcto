...
static void test_two_text_parms(void **state) {
	...
	// DataRow.length + DataRow.num_columns + DataRowParms (each parm: length + string value)
	int32_t expected_length = sizeof(uint32_t) + sizeof(int16_t) + (sizeof(uint32_t)) * num_parms + parm1_length + parm2_length;
...
static void test_three_text_parms(void **state) {
	...
	int32_t expected_length
	    = sizeof(uint32_t) + sizeof(int16_t) + (sizeof(uint32_t)) * num_parms + parm1_length + parm2_length + parm3_length;
...
int main(void) {
	const struct CMUnitTest tests[]
	    = {cmocka_unit_test(test_null_input), cmocka_unit_test(test_zero_parms), cmocka_unit_test(test_one_text_parm),
	       cmocka_unit_test(test_two_text_parms), cmocka_unit_test(test_three_text_parms)};
