...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	; #FUTURE_TODO: Include queries with UPDATE's SET list entry having more versatile expressions than a simple literal value
	...
	set tableAsteriskAggregateExists=0
	...
	set origIsReadWriteMode=isReadWriteMode
	...
	; nameslastname will not have `primaryKeyColumn` defined
	if (0'=$data(primaryKeyColumn)) zwr primaryKeyColumn

	; Pass second argument as `prefix` only
	; Note: The duplicate table name generated by genDuplicateTable() and the name used to define QueryGeneator.m gvn's in
	; 	readZWR() and readSQL() are the same. They are generated using original table name and subtest name. For example,
	;	if table name is `names` and subtest is `TGB01` then the generated table name will be `namesTGB01`.
	if (1=isReadWriteMode) do genDuplicateTable(sqlFile,prefix)

	; SELECT related setup
	; Select maximum of 4 JOINS. Choose 2 with probability ~ 25%, 3 with probability ~ 12.5%, 4 with probability ~ 6.25%.
	...
	; Main Loop
	set file=prefix_".sql"
	; Store isReadWriteMode value as its used in the for loop to communicate to the rest of the code if WRITE query is being
	; generated. The for loop still relies on the original value of isReadWriteMode to know whether this invocation generates
	; only SELECT queries or a mix of SELECT and WRITE queries.
	new lclIsReadWriteMode
	set lclIsReadWriteMode=isReadWriteMode
	for i=1:1:runCount do
	. write "Query Count "_i,!
	. ; select read-only queries
	. if (0=lclIsReadWriteMode)  do
	. . do genSelectQuery()
	. else  do
	. . ; select read-write queries
	. . new randInt
	. . set randInt=$random(100) ; 80% - SELECT queries, 20% - INSERT/UPDATE/DELETE queries
	. . if (80>randInt) do
	. . . set isReadWriteMode=0
	. . . do genSelectQuery()
	. . else  do
	. . . set isReadWriteMode=1
	. . . do genWriteQuery()

	quit

; This routine generates the input and output file names and invokes writeTableDefintion()
; to generate the new table definition queries.
; Input:
; 	fileToReadName - has the path of the .sql file which has the table definition
; 	subtestName- test name used to form the new table definition file name
genDuplicateTable(fileToReadName,subtestName)
	; Parse `fileToReadName` i.e. DDL sql file and generate the following statements to duplicate the tables.`
	...
	new line,nlines,tableNameStart,tableNameEnd,tableName,postgresSqlFile,OctoSqlFile,tableDefinitionFile
	; Setup output file names
	set tableDefinitionFile="tabledefinition-"_subtestName_".sql"
	; Setup input file name
	set postgresInputSqlFile=$select("postgres-nameslastname.sql"=fileToReadName:fileToReadName,1:"postgres-"_fileToReadName)
	set postgresInputSqlFile=$$findFile(postgresInputSqlFile)
	; Read postgres table definition. No GLOBAL here
	do writeTableDefinition(postgresInputSqlFile,tableDefinitionFile,subtestName)
	quit

; Writes new table definition with a table name specific to the subtest being run.
; The table definition written here is executed on both Postgres and Octo.
; Additionally, adds UNIQUE/CHECK constraint to the table definition.
; PRIMARY KEY constraint is not added as the table definition in most cases already has this constraint.
; Input:
;	inputSqlFile - This file contains the original table definition
;	outputSqlFile - This is the file to which the new table definition will be written to
;	subtestName - This value is used to form the new table name for the new table definition
writeTableDefinition(inputSqlFile,outputSqlFile,subtestName)
	new endingChars,offset,line,nlines,endingChars,offset,tableNameStart,tableNameEnd,tableName,restOfTheQueryStart
	new restOfTheQueryEnd,restOfTheQuery,exitLoop,columnNameAndTypeEnd,columnNameAndTypeStart,columnName,columnType
	new columnNameAndType,finalColumnAndType,finalColumnName,finalColumnType,firstColumn,queryComplete,dropTableOutputFileName
	set endingChars=");"
	set offset=1
	; Read original table definition statements
	open inputSqlFile:(readonly)
	use inputSqlFile
	for  read line($increment(nlines)) quit:$zeof
	close inputSqlFile
	kill line(nlines) if $increment(nlines,-1) ; kills line that contains EOF
	; Write new table definition statements with new table name
	set exitLoop=0
	set dropTableOutputFileName="drop-"_outputSqlFile
	new i,firstColumn,tableConstraintAdded
	set firstColumn=1
	set tableConstraintAdded=0
	for i=1:1:nlines do  quit:exitLoop=1
	...
	. . . set:(($length(line(i))=tableNameEnd)!($length(line(i))=(tableNameEnd-1))) tableNameEnd=0
	. . if (($find(line(i),"INTEGER")'=0)!($find(line(i),"VARCHAR")'=0)!($find(line(i),"BOOLEAN")'=0)!($find(line(i),"NUMERIC")'=0)) do
	. . . for  do  quit:((columnNameAndTypeEnd=0)!(($length(line(i))+1)=columnNameAndTypeEnd))
	...
	. . . . . set restOfTheQuery=$$addTableConstraint(restOfTheQuery,.tableConstraintAdded,dupTableName,outputSqlFile)
	. . . . . set firstColumn=0	; This value is later used to add ',' before the next column or constraint definition
	. . . . else  if ($find(line(i),$char(9))'=0) do
	. . . . . ; This is the case where the line is not having an ending ',' character.
	. . . . . ; This is seen mostly in table definition of postgres-composite.sql
	. . . . . set columnNameAndTypeStart=$find(line(i),$char(9),0)	; $char(9) -> tab
	. . . . . set columnNameAndTypeEnd=$length(line(i))
	. . . . . if (($find(line(i),"INTEGER")'=0)!($find(line(i),"VARCHAR")'=0)!($find(line(i),"BOOLEAN")'=0)) do
	. . . . . . set columnNameAndType=$extract(line(i),columnNameAndTypeStart,columnNameAndTypeEnd)
	. . . . . . set columnName=$piece(columnNameAndType," ")
	. . . . . . set columnType=$piece(columnNameAndType," ",2)
	. . . . . . set columnNameAndTypeEnd=0
	. . . . . . set columnNameAndType=$$addConstraint(columnNameAndType,columnName,columnType,dupTableName,outputSqlFile)
	. . . . . . set restOfTheQuery=$select(firstColumn=1:"",1:restOfTheQuery_",")_columnNameAndType
	. . . . . . set firstColumn=0	; This value is later used to add ',' before the next column or constraint definition
	. . . . set tableNameEnd=$select(((($length(line(i))+1)=columnNameAndTypeEnd)):0,1:columnNameAndTypeEnd)
	. . . if ($find(line(i),endingChars)'=0)  do
	. . . . set queryComplete=1
	. . . . set finalColumnAndTypeEnd=$find(line(i),endingChars,columnNameAndTypeStart-1)
	. . . . set finalColumnAndTypeEnd=finalColumnAndTypeEnd-1
	. . . . ; shifted finalcolumnNameAndTypeStart over 1 to remove leading " " shifted
	. . . . ; finalColumnEnd over 2 to remove trailing ") ".
	...
	. . . . set finalColumnAndType=$$addConstraint(finalColumnAndType,finalColumnName,finalColumnType,dupTableName,outputSqlFile)
	. . . . set restOfTheQuery=$select((1=firstColumn):"",1:restOfTheQuery_",")_finalColumnAndType
	. . . . open dropTableOutputFileName:(append)
	. . . . use dropTableOutputFileName
	. . . . write "DROP TABLE IF EXISTS "_dupTableName_";",!
	. . . . close dropTableOutputFileName
	. . . . ; Try to add a table constraint
	. . . . set restOfTheQuery=$$addTableConstraint(restOfTheQuery,.tableConstraintAdded,dupTableName,outputSqlFile)
	. . . . open outputSqlFile:(append)
	. . . . use outputSqlFile
	. . . . write "DROP TABLE IF EXISTS "_dupTableName_";",!
	. . . . write "CREATE TABLE "_dupTableName_" ("_restOfTheQuery_");",!
	. . . . write "INSERT INTO "_dupTableName_" (SELECT * FROM "_tableName_");",!
	. . . . close outputSqlFile
	. . . . set restOfTheQuery=""
	. . . . set firstColumn=1	; variable reset such that the next column definition doesn't prefix ','
	. . else  do  quit:(1=exitLoop)
	. . . if ($find(line(i),"INSERT")'=0) set exitLoop=1 quit
	. . . if (($find(line(i),"UNIQUE")'=0)!($find(line(i),"PRIMARY KEY (")'=0)) do
	. . . . ; This is a UNIQUE or PIMARY KEY constraint definition.
	. . . . set restOfTheQuery=restOfTheQuery_","_$extract(line(i),$find(line(i),$char(9)),$length(line(i))) ; $char(9) -> tab
	. . . . set exitLoop=1
	if (("composite"=tableName)&(0=queryComplete)) do
	. ; Above parsing logic doesn't find the end of composite table definition well
	. ; but at this point all columns have been read so go ahead and print the new
	. ; table definition.
	. open dropTableOutputFileName:(append)
	. use dropTableOutputFileName
	. write "DROP TABLE IF EXISTS "_dupTableName_";",!
...
; This routine is a helper to select the conditions using which addConstraint() can add a constraint
; Here the conditions selected caters to formation of a table level constraint
addTableConstraint(restOfTheQuery,tableConstraintAdded,dupTableName,outputSqlFile)
	if (0=tableConstraintAdded)  do
	. new checkOrUniqueStr,constraintStr
	. set checkOrUniqueStr=$select(0=$random(2):"check",1:"unique")
	. if (($$columnCounter(dupTableName)-1)<2) set singleOrMultipleStr="single"
	. else  set singleOrMultipleStr=$select(0=$random(2):"single",1:"multiple")
	. set constraintStr=$$addConstraint("","","",dupTableName,outputSqlFile,checkOrUniqueStr,singleOrMultipleStr)
	. if (""'=constraintStr) do
	. . set restOfTheQuery=restOfTheQuery_","_constraintStr
	. . set tableConstraintAdded=1
	quit restOfTheQuery

; This routine adds UNIQUE and CHECK constraint to a non-primary-key column.
; If the column passed is found to be a primary-key column then constraint addition is skipped and the passed columnNameAndType
; value is passed unchanged. Otherwise, the returned columnNameAndType value will include a constraint.
; constraintType and constraintParam will only be defined if the invocation is from $$addTableConstraint().
; constraintType - "check","unique"
; constraintParam - "single","multiple"
addConstraint(columnNameAndType,columnName,columnType,tableName,outputSqlFile,constraintType,constraintColumns)
	new randInt
	; Skip constraint addition 80% of the time
	quit:(90>$random(100)) columnNameAndType
	; If the column passed is a primary key skip constraint addition
	quit:(($data(primaryKeyColumn)&($data(primaryKeyColumn(tableName,columnName))))) columnNameAndType
	; Not a primary key, add a constraint
	if ($data(constraintType)) set randInt=$select("unique"=constraintType:0,1:1)	; 0 - UNIQUE, 1 - CHECK
	else  set randInt=$random(2)	; 0 - UNIQUE, 1 - CHECK
	if (0=randInt) do
	. ; Add UNIQUE constraint
	. if (""=columnNameAndType) do
	. . if ("single"=constraintColumns) do
	. . . ; UNIQUE single column
	. . . set columnNameAndType="UNIQUE("_$$chooseColumn(tableName)_")"
	. . else  do
	. . . ; UNIQUE multiple column
	. . . new col1,col2,i
	. . . set col1=$$chooseColumn(tableName)
	. . . for i=1:1 do  quit:(col1'=col2)  do assert(i<64)
	. . . . set col2=$$chooseColumn(tableName)
	. . . set columnNameAndType="UNIQUE("_col1_","_col2_")"
	. else  do
	. . ; UNIQUE at the end of column definition
	. . set columnNameAndType=columnNameAndType_" UNIQUE"
	else  do
	. ; Add CHECK constraint.
	. set expr=$$getCheckConstraint(tableName,columnName,columnType,$select($data(constraintColumns):constraintColumns,1:""))
	. set columnNameAndType=columnNameAndType_expr
	; return columnNameAndType with constraint
	quit columnNameAndType

; Check constraint is added by this routine
getCheckConstraint(tableName,columnName,columnType,constraintColumns)
	; NOTE: aggregate functions and subqueries are not allowed in CHECK)
	new expr,randCase,exprOrder,leftSide,rightSide
	if (""'=constraintColumns) do
	. set randCase=7
	else  do
	...
	. . ; and not by Postgres. This difference in handling causes failures when "''" is used hence
	. . ; the value is replaced by "NULL".
	. . set entry="NULL"
	. else  set entry="FALSE" write columnType	; boolean
	. set leftSide=columnName,rightSide=entry
	else  if (3=randCase) do
	. ; Case 3 - CHECK(col IS NOT NULL)
	. set expr=columnName_" IS NOT NULL"
	else  if (4=randCase) do
	. ; Case 4 -
	. ;   CHECK(col comp_op func(col))
	. ;   CHECK(col comp_op func(col_entry))
	. new funcParm,funcStr
	. set funcParm=$select(0=$random(2):columnName,1:$$chooseEntry(tableName,columnName))
	. set funcStr=$$getRandFunc(funcParm)
	. set leftSide=columnName
	. set rightSide=funcStr
	else  if (5=randCase) do
	. ; Case 5(nullIf/greatest/least) - CHECK(col_entry comp_op func(col_entry,col))
	. new leftParm,rightParm,colEntry,funcExpr,funcStr,randFunc
	. set colEntry=$$chooseEntry(tableName,columnName)
	. set leftParm=$select(0=$random(2):columnName,1:colEntry)
	. set rightParm=$select(colEntry=leftParm:columnName,1:colEntry)
	. set randFunc=$random(3) ; 0-nullif , 1-greatest and 2-least
	. set funcStr=$select(0=randFunc:"nullif",1=randFunc:"greatest",1:"least")
	. set funcExpr=funcStr_"("_leftParm_","_rightParm_")"
	. set leftSide=colEntry,rightSide=funcExpr
	else  if (6=randCase) do
	. ; Case 6 - CHECK(col_entry comp_op coalesce(NULL,col,col_entry)
	. new colEntry,coalesceExpr
	. set colEntry=$$chooseEntry(tableName,columnName)
	. set coalesceExpr="coalesce("_"NULL,"_columnName_","_colEntry_")"
	. set leftSide=colEntry,rightSide=coalesceExpr
	else  if (7=randCase) do
	. ; Case 7 - CHECK(multiple col expr)
	. ; This constraint addition will be done on the table i.e. in a seperate entry not attached to column
	. ; Form the constraints with one or more columns
	. new numCols,genTwoColExpr
	. ; Get total number of columns
	. set numCols=$$columnCounter(tableName)-1
	. ; choose multiple column expr if more than 1 cols exist
	. set genTwoColExpr=$select(numCols<2:0,1:1)
	. ; choose single column expr if less than 2 cols exist
	. if ('genTwoColExpr) do
	. . new column
	. . set column=$$chooseColumn(tableName)
	. . set expr=$$getCheckConstraint(tableName,column,$$getColumnType(tableName,column),"")
	. else  do
	. . new col1,col2,i,expr1,expr2,randExprType
	. . set col1=$$chooseColumn(tableName)
	. . for i=1:1 do  quit:(col1'=col2)  do assert(i<64)
	. . . set col2=$$chooseColumn(tableName)
	. . if ($$getColumnType(tableName,col1)=$$getColumnType(tableName,col2)) set randExprType=$random(2)
	. . else  set randExprType=1
	. . if (0=randExprType) do
	. . . ; Case : (col1 comp_op col2)
	. . . set expr=$select('exprOrder:col1,1:col2)_$$getComparisonOperatorAlone()_$select('exprOrder:col2,1:col1)
	. . else  do
	. . . ; Case : (col1_expr AND/OR col2_expr)
	. . . new col1expr,col2expr,checkPos
	. . . ; Form col1_expr
	. . . set col1expr=$$getCheckConstraint(tableName,col1,$$getColumnType(tableName,col1),"")
	. . . set checkPos=$find(col1expr,"CHECK(")
	. . . set col1expr=$extract(col1expr,checkPos,$length(col1expr)-1)	; -1 to avoid CHECK's closing paren
	. . . ; Form col2_expr
	. . . set col2expr=$$getCheckConstraint(tableName,col2,$$getColumnType(tableName,col2),"")
	. . . set checkPos=$find(col2expr,"CHECK(")
	. . . set col2expr=$extract(col2expr,checkPos,$length(col2expr)-1)	; -1 to avoid CHECK's closing paren
	. . . set expr=$select('exprOrder:col1expr,1:col2expr)_" "_$$booleanOperator_" "_$select('exprOrder:col2expr,1:col1expr)
	else  do
	. ; Case 0
	. ; expr = CHECK(col comp_op col_entry)
	. ; Example: CHECK(col1 <= 4)
	. new colEntry
	. set colEntry=$$chooseEntry(tableName,columnName)
	. set leftSide=columnName,rightSide=colEntry
	if ((7'=randCase)&(3'=randCase)) do
	. set expr=$select(('exprOrder):leftSide,1:rightSide)
	. set expr=expr_$$getComparisonOperatorAlone()
	. set expr=expr_$select(('exprOrder):rightSide,1:leftSide)
	set:(expr'["CHECK") expr=" CHECK("_expr_")"	; The postcondition is for case 7
	quit expr
...
genWriteQuery()
	; Select whether to generate INSERT/UPDATE/DELETE query
	new randInt
	set aliasNum=0	; Required as some of the calls below expect this variable to be defined
...
genInsertQuery()
	; The following are the grammar rules which form the INSERT query. Generate queries conforming to these rules.
	; INSERT INTO table_name ( column name [, column name ...]) [ VALUES ... | (SELECT ...)];
	; (1) INSERT INTO column_name query_expression
	; (2) INSERT INTO column_name LEFT_PAREN column_name_list RIGHT_PAREN query_expression
	; (3) -- INSERT INTO column_name DEFAULT VALUES -- #FUTURE_TODO: YDBOcto#555
	; column_name -> table name -> call chooseTable() to get the table on which the query should act on
	new tableName,insertQuery
	set tableName=$$chooseTable
	set insertQuery="INSERT INTO "_tableName_" "
	; Choose whether to generate a query which matches rule (1) or (2)
	new randInt
	set randInt=$random(2)
	if (0=randInt) do
	. ; Generating query which matches rule (1)
	. ; Create an insertColumns lvn which informs the rest of the code the type of column values being expected
	. new totalColumns,i,insertColumns
	. set totalColumns=$$columnCounter(tableName)
	. for i=1:1:totalColumns-1 do
	. . ; Add table name, column name, type and Key or not info
	. . new lclColName,lclColNameType,lclIsKey
	. . set lclColName=""
	. . set lclColName=$ORDER(columns(tableName,i,lclColName))
	. . set lclColNameType=columns(tableName,i,lclColName)
	. . set lclIsKey=$select((0'=$GET(primaryKeyColumn(tableName,lclColName),0)):1,1:0)
	. . set insertColumns(tableName,i,lclColName,lclColNameType,lclIsKey)=""
	. ; Generate query_expression and append it to `insertQuery`
	. set insertQuery=insertQuery_$$insertValuesClause($$columnCounter(tableName)-1)
	else  do
	. ; Generating query which matches rule (2)
	. ; Generate LEFT_PAREN column_name_list RIGHT_PAREN query_expression and append it to `insertQuery`
	. ; Conditions:
	. ; 	* column_name_list can have any order of elements i.e. doesn't have to be col3 after col2 after col1
	. ;	* column_name_list doesn't have to list all columns of the table. Columns not listed receive NULL as its value
	. ;	* column_name_list has to include primary key if it is present in the table
	. ;	* column_name_list type must match with the values which are to be inserted
	. ;	* column_name_list column count must match the query expression column count
	. ;	* column_name_list cannot have multiple entry of the same column
	. ;	* Values being inserted to Primary key column cannot be a duplicate but for a regular column duplicate values are
	. ;	    allowed
...
insertValuesClause(numCols)
	...
	set numRows=$random(5)+1
	for i=1:1:numRows do
	. set valuesClause=valuesClause_"("
	. ; Generate column values for all the columns seen in `insertColumns`
	. new firstColumn,i2
	. set columnName=""
	. set firstColumn=1
	. for i2=1:1:numCols do
	. . if (0=firstColumn) set valuesClause=valuesClause_","
	. . else  set firstColumn=0
	. . new colType,isKey,columnName
	. . set columnName=""
	. . set columnName=$order(insertColumns(tableName,i2,columnName))
	. . set colType=""
	. . set colType=$order(insertColumns(tableName,i2,columnName,colType))
	. . ;	Select a random column value from the selected column of the original table
	. . set colValueToInsert=$$chooseEntry(tableName,columnName)
	. . set valuesClause=valuesClause_colValueToInsert
	. set valuesClause=valuesClause_")"
	. set:(numRows'=i) valuesClause=valuesClause_","
	set valuesClause=valuesClause_")"
	quit valuesClause

genUpdateQuery()
	; Following rules are possible for UPDATE clause
	; 1. UPDATE column_name SET set_clause_list
	; 2. UPDATE column_name optional_as as_name SET set_clause_list
	; 3. UPDATE column_name SET set_clause_list WHERE search_condition
	; 4. UPDATE column_name optional_as as_name SET set_clause_list WHERE search_condition
	; 1 and 3 are implemented by this routine
	; Note multiple assignments to same columns is not allowed
	new updateQuery,tableName,updateColumns
	set tableName=$$chooseTable
	set updateQuery="UPDATE "_tableName_" "

	; Form the SET
	set updateQuery=updateQuery_"SET "
	set updateSetList=""
	new randInt,i,commaRequired,columnName,endOfLoop
	set commaRequired=0
	set endOfLoop=0
	set randInt=$random(3)+1 ; number of set_clause_list elements
	for i=1:1:randInt do  quit:(1=endOfLoop)
	. ; form a set_clause_list
	. ; Left should be a column_name
	. ; Right should be an expression
	. ; column is set with the expression
	. new columnIndex,columnType
	. set columnName=$$chooseColumn(tableName)
	. ; We avoid updating a primary column as that requires state maintainence
	. ; which is difficult in this case.
	. quit:(($data(primaryKeyColumn)&($data(primaryKeyColumn(tableName,columnName)))))
	. if (0=$data(updateColumns(columnName))) do  quit:(1=endOfLoop)
	. . set updateColumns(columnName)=""
	. . set:(1=commaRequired) updateSetList=updateSetList_","
	. . set columnIndex=$$returnColumnIndex(tableName,columnName)
	. . set columnType=columns(tableName,columnIndex,columnName)
	. . set updateSetList=updateSetList_$$getColumnUpdateString(tableName,columnName,columnType)
	. . set commaRequired=1
	; Choose whether to have WHERE clause or not
	if (""'=updateSetList) do
	. set updateQuery=updateQuery_updateSetList
	. set:(1=$random(2)) updateQuery=updateQuery_" "_$$whereClause(0,tableName)
	. set updateQuery=updateQuery_";"
	. open file:(append)
	. use file
	. write updateQuery,!
	. close file
...
getColumnUpdateString(tableName,columnName,columnType)
	; Case 0 - col_entry
	; Case 1 - column based expression
	; 		col+1
	;		col||'test'
	;		col||NULL
	;		(SELECT above_type_expr)
	; Case 2 - function
	; Case 3 - Greatest/Least/NULLIF/Coalesce
	; Case 4 - Case statement
	;		col and col_entry based
	; Case 5 - subquery
	new setClauseListEntry,randCase,expr,needsParen
	set randCase=$random(4),needsParen=0
	if (1=randCase) do
	. new leftSide,rightSide,oper
	. if ("INTEGER"=columnType) do
	. . set leftSide=$select(1=$random(2):columnName,1:$$getRandInt())
	. . set rightSide=$select(1=$random(2):columnName,1:$$getRandInt())
	. . set oper=$$arithmeticOperator()
	. else  if ("NUMERIC"=columnType) do
	. . set leftSide=$select(1=$random(2):columnName,1:$$getRandInt()_"."_$$getRandInt())
	. . set rightSide=$select(1=$random(2):columnName,1:$$getRandInt()_"."_$$getRandInt())
	. . set oper=$$arithmeticOperator()
	. else  if (columnType["VARCHAR") do
	. . ; Using NULL instead of a randomely generated value or the column itself as concatenation can lead to
	. . ; a string size greater than the defined size for the column.
	. . ; Also NULL is used instead of an empty string because Octo treats empty string as NULL
	. . ; where as Postgres doesn't. This difference causes queries to generate different output. So, NULL is used
	. . ; instead to avoid test failures.
	. . set leftSide=$select(1=$random(2):columnName,1:"NULL")
	. . set rightSide=$select(leftSide=columnName:"NULL",1:columnName)
	. . set oper="||"
	. else  do
	. . ; BOOLEAN
	. . set leftSide=$select(1=$random(2):columnName,1:$$getRandBoolean)
	. . set rightSide=$select(1=$random(2):columnName,1:$$getRandBoolean)
	. . set oper=$$booleanOperator()
	. . set needsParen=1
	. ; Cannot have both Sides NULL as Postgres generates error for NULL - NULL operation
	. ; set:(1=$random(8)) leftSide="NULL"
	. set:(1=$random(8)) rightSide="NULL"
	. set:("%"=oper) rightSide="("_rightSide_"+1)"	; Avoid divide by zero error
	. set expr=leftSide_" "_oper_" "_rightSide
	. set:1=$random(4) expr="(SELECT "_expr_")" ;
	. if needsParen do
	. . ; Parenthesis is required in case of BOOLEAN operation as Octo doesn't read boolean expressions well
	. . ; without the parenthesis. Ex:`SET available=available OR available;`
	. . set expr="("_expr_")"
	else  if (2=randCase) do
	. set expr=$$getRandFunc($select(1=$random(2):columnName,1:$$chooseEntry(tableName,columnName)))
	else  if (3=randCase) do
	. new randSubCase,funcStr,colEntry1,colEntry2
	. set randSubCase=$random(4)
	. set funcStr=$select(0=randSubCase:"greatest",1=randSubCase:"least",2=randSubCase:"nullif",1:"coalesce")
	. if ("nullif"=funcStr) do
	. . set colEntry1=$$chooseEntry(tableName,columnName)
	. . set colEntry2=columnName
	. else  do
	. . set randSubCase=$random(2)
	. . set colEntry1=$select(0=randSubCase:columnName,1:$$chooseEntry(tableName,columnName))
	. . set colEntry2=$select(0=randSubCase:columnName,1:$$chooseEntry(tableName,columnName))
	. set expr=funcStr_"("_colEntry1_","_colEntry2_")"
	else  if (4=randCase) do
	. ; #FUTURE_TODO: $$returnCaseFunction needs to be updated to be able to choose columns
	. ; 		  which are compatible with size as well. Once it is implemented this case can be enabled.
	. new toCompare
	. set toCompare=tableName_"."_columnName
	. set leftCaseArg=$$chooseEntry(tableName,columnName)
	. set rightCaseArg=$$chooseEntry(tableName,columnName)
	. set table=tableName
	. set expr=$$returnCaseFunction("UPDATE","lrComparison","columns","FALSE",toCompare)
	else  if (5=randCase) do
	. ; #FUTURE_TODO: Enable this case once generateSubQuery() is updated to be able to generate a subquery
	. ; 		  with column of the type which can be requested by the caller.
	. set expr=$$generateSubQuery(0,"limited")
	else  set expr=$$chooseEntry(tableName,columnName)
	set setClauseListEntry=columnName_"="_expr
	quit setClauseListEntry
...
genDeleteQuery()
	...
	set tableName=$$chooseTable
	set deleteQuery="DELETE FROM "_tableName_" "
	; generate WHERE clause
	set deleteQuery=deleteQuery_$$whereClause(0,tableName)_";"
	open file:(append)
	use file
	write deleteQuery,!
	close file
...
fillCompositeSql()
	new databaseName
	set databaseName=$SELECT((1=isReadWriteMode):"composite"_prefix,1:"composite")
	set sqlInfo(databaseName,1,"id0","INTEGER")=""
	set sqlInfo(databaseName,2,"id1","INTEGER")=""
	set sqlInfo(databaseName,3,"id2","INTEGER")=""
	set sqlInfo(databaseName,4,"id3","INTEGER")=""
	set sqlInfo(databaseName,5,"id4","INTEGER")=""
	set sqlInfo(databaseName,6,"id5","INTEGER")=""
	set sqlInfo(databaseName,7,"id6","INTEGER")=""
	set sqlInfo(databaseName,8,"id7","INTEGER")=""
	set sqlInfo(databaseName,9,"name","VARCHAR(20)")=""
	set primaryKeyColumn(databaseName,"id0")="INTEGER"
	set primaryKeyColumn(databaseName,"id1")="INTEGER"
	set primaryKeyColumn(databaseName,"id2")="INTEGER"
	set primaryKeyColumn(databaseName,"id3")="INTEGER"
	set primaryKeyColumn(databaseName,"id4")="INTEGER"
	set primaryKeyColumn(databaseName,"id5")="INTEGER"
	set primaryKeyColumn(databaseName,"id5")="INTEGER"
	set primaryKeyColumn(databaseName,"id6")="INTEGER"
	set primaryKeyColumn(databaseName,"id7")="INTEGER"
...
readSQL(file)
	...
	. . . . set:($find(finalColumnAndType,"KEY")'=0) primaryKeyColumn(tableName,finalColumnName)=finalColumnType
...
readZWR(file)
	new line,nlines,holder,firstData,table,i,i2,previous,databaseGblName
	open file:(readonly)
	use file
		for  read line($increment(nlines))  quit:$zeof
	close file
	...
	. . . . set databaseGblName=$SELECT((1=isReadWriteMode):$piece(fullThing,"(")_prefix,1:$piece(fullThing,"("))
	. . . . set table=databaseGblName_"("_innerNumber_")" ; EX: ^Names(custom number)
...
fillDefaultNamesLastnameTbl()
	new databaseName
	set databaseName=$SELECT((1=isReadWriteMode):"nameslastname"_prefix,1:"nameslastname")
	set data("^"_databaseName_"(1)",1,"Cool")=""
	set data("^"_databaseName_"(2)",1,"Burn")=""
	set data("^"_databaseName_"(3)",1,"Killer")=""
	set data("^"_databaseName_"(4)",1,"Nikon")=""
	set data("^"_databaseName_"(5)",1,"")=""
	set data("^"_databaseName_"(6)",1,"Cool")=""
	quit
...
fillDefaultCompositeTbl()
	...
	set databaseName=$SELECT((1=isReadWriteMode):"composite"_prefix,1:"composite")
	; 1st row
	set data("^"_databaseName_"(1)",1,0)=""
	set data("^"_databaseName_"(1)",2,1)=""
	set data("^"_databaseName_"(1)",3,2)=""
	set data("^"_databaseName_"(1)",4,3)=""
	set data("^"_databaseName_"(1)",5,4)=""
	set data("^"_databaseName_"(1)",6,5)=""
	set data("^"_databaseName_"(1)",7,6)=""
	set data("^"_databaseName_"(1)",8,7)=""
	set data("^"_databaseName_"(1)",9,"Name1")=""

	; 2nd row
	set data("^"_databaseName_"(2)",1,0)=""
	set data("^"_databaseName_"(2)",2,1)=""
	set data("^"_databaseName_"(2)",3,2)=""
	set data("^"_databaseName_"(2)",4,3)=""
	set data("^"_databaseName_"(2)",5,4)=""
	set data("^"_databaseName_"(2)",6,5)=""
	set data("^"_databaseName_"(2)",7,6)=""
	set data("^"_databaseName_"(2)",8,8)=""
	set data("^"_databaseName_"(2)",9,"Name2")=""

	; 3rd row
	set data("^"_databaseName_"(3)",1,0)=""
	set data("^"_databaseName_"(3)",2,1)=""
	set data("^"_databaseName_"(3)",3,2)=""
	set data("^"_databaseName_"(3)",4,3)=""
	set data("^"_databaseName_"(3)",5,4)=""
	set data("^"_databaseName_"(3)",6,5)=""
	set data("^"_databaseName_"(3)",7,7)=""
	set data("^"_databaseName_"(3)",8,7)=""
	set data("^"_databaseName_"(3)",9,"Name3")=""

	; 4th row
	set data("^"_databaseName_"(4)",1,0)=""
	set data("^"_databaseName_"(4)",2,1)=""
	set data("^"_databaseName_"(4)",3,2)=""
	set data("^"_databaseName_"(4)",4,3)=""
	set data("^"_databaseName_"(4)",5,4)=""
	set data("^"_databaseName_"(4)",6,5)=""
	set data("^"_databaseName_"(4)",7,8)=""
	set data("^"_databaseName_"(4)",8,7)=""
	set data("^"_databaseName_"(4)",9,"Name4")=""

	; 5th row
	set data("^"_databaseName_"(5)",1,0)=""
	set data("^"_databaseName_"(5)",2,1)=""
	set data("^"_databaseName_"(5)",3,2)=""
	set data("^"_databaseName_"(5)",4,3)=""
	set data("^"_databaseName_"(5)",5,4)=""
	set data("^"_databaseName_"(5)",6,6)=""
	set data("^"_databaseName_"(5)",7,8)=""
	set data("^"_databaseName_"(5)",8,7)=""
	set data("^"_databaseName_"(5)",9,"Name5")=""

	; 6th row
	set data("^"_databaseName_"(6)",1,0)=""
	set data("^"_databaseName_"(6)",2,1)=""
	set data("^"_databaseName_"(6)",3,2)=""
	set data("^"_databaseName_"(6)",4,3)=""
	set data("^"_databaseName_"(6)",5,5)=""
	set data("^"_databaseName_"(6)",6,6)=""
	set data("^"_databaseName_"(6)",7,8)=""
	set data("^"_databaseName_"(6)",8,7)=""
	set data("^"_databaseName_"(6)",9,"Name6")=""

	; 7th row
	set data("^"_databaseName_"(7)",1,0)=""
	set data("^"_databaseName_"(7)",2,1)=""
	set data("^"_databaseName_"(7)",3,2)=""
	set data("^"_databaseName_"(7)",4,4)=""
	set data("^"_databaseName_"(7)",5,5)=""
	set data("^"_databaseName_"(7)",6,6)=""
	set data("^"_databaseName_"(7)",7,8)=""
	set data("^"_databaseName_"(7)",8,7)=""
	set data("^"_databaseName_"(7)",9,"Name7")=""

	; 8th row
	set data("^"_databaseName_"(8)",1,0)=""
	set data("^"_databaseName_"(8)",2,1)=""
	set data("^"_databaseName_"(8)",3,3)=""
	set data("^"_databaseName_"(8)",4,4)=""
	set data("^"_databaseName_"(8)",5,5)=""
	set data("^"_databaseName_"(8)",6,6)=""
	set data("^"_databaseName_"(8)",7,8)=""
	set data("^"_databaseName_"(8)",8,7)=""
	set data("^"_databaseName_"(8)",9,"Name8")=""

	; 9th row
	set data("^"_databaseName_"(9)",1,0)=""
	set data("^"_databaseName_"(9)",2,2)=""
	set data("^"_databaseName_"(9)",3,3)=""
	set data("^"_databaseName_"(9)",4,4)=""
	set data("^"_databaseName_"(9)",5,5)=""
	set data("^"_databaseName_"(9)",6,6)=""
	set data("^"_databaseName_"(9)",7,8)=""
	set data("^"_databaseName_"(9)",8,7)=""
	set data("^"_databaseName_"(9)",9,"Name9")=""

	; 10th row
	set data("^"_databaseName_"(10)",1,1)=""
	set data("^"_databaseName_"(10)",2,2)=""
	set data("^"_databaseName_"(10)",3,3)=""
	set data("^"_databaseName_"(10)",4,4)=""
	set data("^"_databaseName_"(10)",5,5)=""
	set data("^"_databaseName_"(10)",6,6)=""
	set data("^"_databaseName_"(10)",7,8)=""
	set data("^"_databaseName_"(10)",8,7)=""
	set data("^"_databaseName_"(10)",9,"Name10")=""
	quit
...
; Called by whereClause and innerWhereClause
; set innerTable=$piece(innerFC," ",2)
;	if (0=isReadWriteMode) do
;	. set table=$piece(fc," ",2)
;	else  do
;	. set table=tableName
; in case of innerquery call
; set table="alias"_aliasNum
whereExpressionHelper(queryDepth,table,innerTable,randInt,result,whereType)
	write "whereExpressionHelper(): queryDepth=",queryDepth," table=",table," innerTable=",innerTable," randInt=",randInt," result=",result," whereType=",whereType,!
	if (randInt=0) do
	...
	. set result=result_plusMinus_$$chooseEntry(innerTable,chosenColumn)_")="_$$getRandFuncInt($$chooseEntry(innerTable,chosenColumn))_")"
	...
	. set result=result_$$comparisonOperators(table_"."_chosenColumn,plusMinus_"("_plusMinus2_$$chooseEntry(innerTable,chosenColumn)_")")
	...
	. new chosenColumn
	. set chosenColumn=$$chooseColumn(innerTable)
	. set result=result_table_"."_chosenColumn_" BETWEEN "_$$chooseEntry(innerTable,chosenColumn)_" AND "_$$chooseEntry(innerTable,chosenColumn)
	...
	.
	. if ("whereClause"=whereType) do
	. . set leftSide=innerTable_"."_leftSide
	. else  do
	. . set leftSide="alias"_(aliasNum1More-1)_"."_leftSide
	.
	. if (i'=15) do
	. . set result=result_$$comparisonOperators(leftSide,word_" "_rightSide)
	. else  set result=""
	; From this point changes are different
	...
	write "whereExpressionHelper(): result=",result,!
	quit result
...
; `tableName` is only defined for invocation from WRITE query generation routines
whereClause(queryDepth,tableName)
	do assert(enableWhereClause!isReadWriteMode)
	...
	quit $$whereExpressionHelper(queryDepth,table,innerTable,randInt,result,"whereClause")
...
generateQuery(queryDepth,joinCount)
	...
	if ("names.sql"=sqlFile)  do
	. set fc=fc_" AS names"
	. set:(1=origIsReadWriteMode) fc=fc_prefix
...
generateSubQuery(queryDepth,subQueryType,joinType)
	...
	new tableColumnSave,selectListLVNSave,tableAliasSave,groupByListLVNSave,groupByColumnLVNSave,selectedExprGroupBySave
	new groupByComparisonExpressionExistsSave,tableAsteriskAggregateExistsSave
	...
	merge tableAsteriskAggregateExistsSave=tableAsteriskAggregateExists
	...
	kill tableColumn,selectListLVN,tableAlias,groupByListLVN,groupByColumnLVN,selectedExprGroupBy
	kill groupByComparisonExpressionExists,tableAsteriskAggregateExists
	...
	merge tableAsteriskAggregateExists=tableAsteriskAggregateExistsSave
...
innerWhereClause(queryDepth)
	...
	set table="alias"_aliasNum
	quit $$whereExpressionHelper(queryDepth,table,innerTable,randInt,result,"innerWhereClause")
