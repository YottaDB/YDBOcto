...
-- Others
SELECT orders.order_date,COUNT(alias1.*) FROM orders  LEFT JOIN orders AS alias1 ON ((orders.order_date::varchar = alias1.order_amount) AND NOT ((orders.customer_id > alias1.customer_id) AND NOT (orders.order_amount < alias1.order_date::varchar))) WHERE ((3 > 1) OR ((orders.order_date = date'1790-09-03'))) GROUP BY alias1.*, orders.order_date HAVING COUNT(DISTINCT alias1.*) IN (1);
...
SELECT alias1.address,COUNT(alias2.*) FROM orders  RIGHT JOIN customers AS alias1 ON (((orders.order_amount = alias1.address) AND NOT (orders.order_date::varchar = alias1.first_name)) OR (orders.order_id != 4)) LEFT OUTER JOIN orders AS alias2 ON ((('Charlottesville' < '09/03/1790'))) WHERE orders.order_date BETWEEN date'1784-05-23' AND date'1784-05-23' GROUP BY alias2.*, alias1.address HAVING COUNT(DISTINCT alias2.*) NOT IN (1) ORDER BY COUNT(alias2.*), alias1.address;
