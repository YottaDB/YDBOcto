...
SELECT count(DISTINCT alias1.*) FROM orders  LEFT JOIN orders AS alias1 ON (orders.order_date = alias1.order_amount);
-- Others
SELECT orders.order_date,COUNT(alias1.*) FROM orders  LEFT JOIN orders AS alias1 ON ((orders.order_date = alias1.order_amount) AND NOT ((orders.customer_id > alias1.customer_id) AND NOT (orders.order_amount < alias1.order_date))) WHERE ((3 > 1) OR ((orders.order_date = '09/03/1790'))) GROUP BY alias1.*, orders.order_date HAVING COUNT(DISTINCT alias1.*) IN (1);
...
SELECT alias1.address,COUNT(alias2.*) FROM orders  RIGHT JOIN customers AS alias1 ON (((orders.order_amount = alias1.address) AND NOT (orders.order_date = alias1.first_name)) OR (orders.order_id != 4)) LEFT OUTER JOIN orders AS alias2 ON ((('Charlottesville' < '09/03/1790'))) WHERE orders.order_date BETWEEN '05/23/1784' AND '05/23/1784' GROUP BY alias2.*, alias1.address HAVING COUNT(DISTINCT alias2.*) NOT IN (1) ORDER BY COUNT(alias2.*), alias1.address;
SELECT ALL alias6.first_name,COUNT(alias6.*) FROM orders  RIGHT JOIN (SELECT alias1.city, alias1.zipcode, alias1.last_name FROM customers alias1) AS alias1 ON (((orders.order_amount < SOME (SELECT DISTINCT alias2.order_amount FROM orders alias2 ORDER BY alias2.order_amount LIMIT 1))) OR NOT (orders.customer_id != ANY (SELECT DISTINCT alias3.customer_id FROM customers alias3 ORDER BY alias3.customer_id LIMIT 1)) OR NOT (orders.order_date = ANY (SELECT DISTINCT alias4.address FROM customers alias4 ORDER BY alias4.address LIMIT 1))) RIGHT JOIN (SELECT alias6.email, alias6.first_name, alias6.zipcode FROM customers alias6) AS alias6 ON (((orders.customer_id = ANY (SELECT DISTINCT alias7.order_id FROM orders alias7 ORDER BY alias7.order_id LIMIT 1))) AND NOT (orders.order_amount != alias6.email) AND NOT (orders.order_date != ALL (SELECT DISTINCT alias8.order_amount FROM orders alias8 ORDER BY alias8.order_amount LIMIT 1))) RIGHT JOIN orders AS alias10 ON ((orders.order_id != alias10.order_id)) WHERE (NOT (3 < 2)) GROUP BY alias6.*, alias6.first_name HAVING COUNT(DISTINCT alias6.*) BETWEEN 1 AND 6;
