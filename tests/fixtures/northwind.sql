...
CREATE TABLE Orders (OrderID INTEGER PRIMARY KEY, CustomerID INTEGER, EmployeeID INTEGER, OrderDate DATE, ShipperID INTEGER) GLOBAL "^Orders(keys(""OrderID""))";
...
CREATE TABLE Products (ProductID INTEGER PRIMARY KEY, ProductName VARCHAR(32), SupplierID INTEGER, CategoryID INTEGER, Unit VARCHAR(32), Price NUMERIC) GLOBAL "^Products(keys(""ProductID""))";
...
CREATE TABLE Employees (EmployeeID INTEGER PRIMARY KEY, LastName VARCHAR(32), FirstName VARCHAR(32), BirthDate DATE, Photo VARCHAR(16), Notes VARCHAR(128)) GLOBAL "^Employees(keys(""EmployeeID""))";
