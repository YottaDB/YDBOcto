...
func main() {
  ...
  // Update 2 rows in names table
  lastName := "Cool"	// Use variable instead of hardcoded constant in query in order to exercise extended query protocol
  rows, err = db.Query("UPDATE names set firstname = lastname, lastname = firstname where lastname = $1", lastName)
  if err != nil {
    fmt.Printf("%v\n", err)
    panic(err)
  }
  ...
  // Verify names table has 6 rows but with firstname/lastname swapped in 2 rows
  rows, err = db.Query("SELECT * FROM names")
  if err != nil {
    fmt.Printf("%v\n", err)
    panic(err)
